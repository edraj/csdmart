    ...
    public async Task<string?> MintAsync(User user, InvitationChannel channel, bool isReset, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // three flow through Scriban with the same {name, msisdn,
                // shortname, link} scope so operators can reference any of
                // those vars in any field. SmtpSender composes a
                // multipart/alternative message when both bodies are
                // non-empty and falls back to whichever side is present
