    ...
    ILogger<ImportExportService> log)
{
        ...
        ZipArchive zip, string spaceName, Entry parent, string parentMetaDir, CancellationToken ct)
    {
        ...
        //
        // includeMedia: true is REQUIRED here. ListForParentAsync defaults to the
        // media-less projection (NULL::bytea) because the listing/query paths only
        // ever render metadata — but export is the third consumer, and it writes the
        // raw bytes into the zip below. With the default projection att.Media is
        // always null, so the media branch never fires and every export silently
        // ships attachment metadata with no file behind it.
        var attList = await attachments.ListForParentAsync(
            spaceName, parent.Subpath, parent.Shortname, ct, includeMedia: true);
