...
public sealed class LogSink : IDisposable
{
    ...
    public void WriteLog(string category, LogLevel level, string message)
    {
        ...
        var record = new Dictionary<string, object?>
        {
            ...
            // overrides either, this line renders differently from the same
            // format string as written by a plugin into its own
            // `<shortname>.ljson.log`, and the two log streams stop interleaving
            // cleanly. The format is a wire contract, not display text.
