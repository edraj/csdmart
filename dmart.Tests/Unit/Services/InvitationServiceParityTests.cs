...
public class InvitationServiceParityTests
{
    ...
    {
        // The text template is plain text; mail clients render it as text/
        // plain, so escaping is unnecessary (and harmful — recipients would
        // see literal "&lt;" instead of "<"). Hostile substrings appear
        // verbatim; this is safe because no rendering happens.
        var user = NewUser(shortname: "<script>alert('xss')</script>",
            displayname: new Translation(En: "<img src=x onerror=alert(1)>"));
