...
// changes from racing across env-mutating overlay tests.
...
public sealed class LanguageLoaderOverlayTests : IDisposable
{
    ...
    {
        // Override `otp_message` for English — every other embedded key
        // (the workflow state labels, ...) must remain intact.
        File.WriteAllText(
