...
internal sealed class FakeDataGenerator(int seed)
{
    // ---- deterministic PRNG -------------------------------------------------
    //
    // Hand-rolled xorshift64* rather than System.Random, for two reasons:
    //
    //   * STABILITY. System.Random's seeded sequence is explicitly not
    //     contracted to be stable across .NET releases. Since this file's whole
    //     point is that re-running --seed reproduces the same sample data, a
    //     runtime upgrade silently rewriting every generated row would defeat
    //     it. Nine lines of arithmetic are fixed forever.
    //
    //   * ANALYZERS. CA5394 flags System.Random wholesale as insecure, which is
    //     correct advice in general and irrelevant here — nothing generated is
    //     security-relevant. Owning the generator is cleaner than sprinkling
    //     suppressions, and a crypto RNG could not be seeded reproducibly
    //     anyway, so it would break the requirement above.
    //
    // NOT for anything security-sensitive: no keys, tokens, passwords or IDs
    // that must be unguessable. Sample content only.
    private ulong _state = Seed(seed);

    // xorshift64 needs a non-zero state, and a small seed leaves the first few
    // outputs poorly mixed — so the seed goes through a SplitMix64 round first.
    private static ulong Seed(int seed)
    {
    ...
    /// <summary>Uniform in [0, <paramref name="maxExclusive"/>).</summary>
    private int Next(int maxExclusive)
        => maxExclusive <= 0 ? 0 : (int)(NextBits() % (uint)maxExclusive);

    /// <summary>Uniform in [<paramref name="min"/>, <paramref name="maxExclusive"/>).</summary>
    private int Next(int min, int maxExclusive)
        => maxExclusive <= min ? min : min + Next(maxExclusive - min);

    /// <summary>Uniform in [0, 1). 24 bits of mantissa, which is ample here.</summary>
    private double NextDouble() => (NextBits() & 0xFFFFFF) / (double)0x1000000;

    private void NextBytes(byte[] bytes)
    {
        for (var i = 0; i < bytes.Length; i++) bytes[i] = (byte)NextBits();
    }
    ...
    private JsonNode? Build(JsonElement schema, int depth)
    {
            ...
            && enumVal.ValueKind == JsonValueKind.Array)
        {
            ...
            if (choices.Count > 0) return JsonNodeFrom(choices[Next(choices.Count)]);
        }
        ...
        return TypeOf(schema) switch
        {
            ...
            "boolean" => JsonValue.Create(Next(2) == 1),
            "null" => null,
    ...
    private JsonArray BuildArray(JsonElement schema, int depth)
    {
        ...
        var count = min >= max ? min : Next(min, max + 1);
    ...
    private string BuildString(JsonElement schema)
    {
        ...
        var value = format switch
        {
            ...
            "ipv4" => $"10.{Next(256)}.{Next(256)}.{Next(1, 255)}",
            _ => $"{Word()}-{Next(1000)}",
        };
    ...
    private long BuildInteger(JsonElement schema)
    {
        ...
        var value = min == max ? min : min + (long)(NextDouble() * (max - min));
    ...
    private double BuildNumber(JsonElement schema)
    {
        ...
        return Math.Round(min + NextDouble() * (max - min), 2);
    }

    private string Word() => Words[Next(Words.Length)];
    ...
    private DateTime Date() => new DateTime(2024, 1, 1, 0, 0, 0, DateTimeKind.Utc)
        .AddDays(Next(730)).AddSeconds(Next(86400));
